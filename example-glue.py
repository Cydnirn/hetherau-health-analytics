...
for col in medical_cols:
    median_val = df.filter(F.col(col) > 0).approxQuantile(col, [0.5], 0.001)[0]
    logger.info(f"  {col:25s}: median = {median_val:.3f}")
    df = df.withColumn(col, F.when(F.col(col) == 0, median_val).otherwise(F.col(col)))
...
# Interaction feature
df = df.withColumn(
    "glucose_bmi_interaction", (F.col("glucose") * F.col("bmi") / 1000.0)
)

logger.info("  Feature engineering complete. Sample:")
df.select(
    "bmi_category", "age_group", "glucose_risk", "glucose_bmi_interaction", "outcome"
...
logger.info(
    "  Normalization stats saved to s3://DEST_BUCKET/config/normalization_stats.json"
)
...
logger.info(
    f"  Norm stats:         s3://{args['DEST_BUCKET']}/config/normalization_stats.json"
)
logger.info("=" * 60)
