...
STUDENT_NAME = "cydnirn"  # ← CHANGE THIS
ACCOUNT_ID = boto3.client("sts").get_caller_identity()["Account"]
REGION = "us-east-1"
ROLE_ARN = f"arn:aws:iam::{ACCOUNT_ID}:role/LabRole"

PROJECT = "healthpredict"
DATASET_BUCKET = f"healthpredict-s3-cyd-2026"
MODELS_BUCKET = f"healthpredict-s3-cyd-2026"
PIPELINE_NAME = f"{PROJECT}-training-pipeline"
MODEL_GROUP = f"{PROJECT}-model-group"

session = sagemaker.Session(boto_session=boto3.Session(region_name=REGION))
pipeline_session = PipelineSession(boto_session=boto3.Session(region_name=REGION))
...
def build_pipeline():
    ...
    # Chain dependencies
    # training_step depends on processing_step via property references (implicit)
    # register_step depends on training_step via model_data property reference (implicit)
    # ModelStep is a StepCollection and does not support add_depends_on;
    # explicit dep for TrainingStep is kept for clarity.
